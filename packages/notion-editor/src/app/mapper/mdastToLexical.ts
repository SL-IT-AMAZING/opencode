...
export function importMarkdownToLexical(editor: LexicalEditor, root: Root): void {
  editor.update(() => {
...
function convertListItem(node: ListItem, parentList: List): ListItemNode {
  const listItem = $createListItemNode(parentList.ordered === false && node.checked !== null ? node.checked : undefined)
...
function convertTableRow(node: TableRow, isHeader: boolean, alignments: Table["align"]): TableRowNode {
